...
@main_bp.route("/debug_check_user_immediately/<email>")
def debug_check_user_immediately(email):
    """Check user in database immediately after creation"""
    from app.models.user import Users
    from sqlalchemy import text
    
    # Check via SQLAlchemy ORM
    user_orm = Users.query.filter_by(email=email).first()
    
    # Check via raw SQL to bypass any ORM caching
    result = db.session.execute(text('SELECT * FROM "Users" WHERE email = :email'), {'email': email})
    user_sql = result.fetchone()
    
    return jsonify({
        'email': email,
        'orm_found': bool(user_orm),
        'sql_found': bool(user_sql),
        'orm_details': {
            'id': user_orm.id if user_orm else None,
            'name': user_orm.name if user_orm else None,
            'email': user_orm.email if user_orm else None,
            'role_id': user_orm.role_id if user_orm else None
        } if user_orm else None,
        'sql_details': dict(user_sql) if user_sql else None
    })



@main_bp.route("/debug_user_creation/<email>")
def debug_user_creation(email):
    ...
    org_contact = OrganizationContacts.query.filter_by(email=email).first()
    
    # Try to create user manually
    from utils.user_helpers import create_minimal_user, create_user_direct_sql
    import secrets
    
    result = {
        'current_state': {
            'user_exists': bool(user),
            'org_contact_exists': bool(org_contact),
            'user_details': {
    ...
    if org_contact:
        ...
        result['test_creation']['minimal'] = bool(test_user)
        
        if test_user:
            db.session.rollback()  # Don't actually save
        else:
            # Test SQL creation
            test_user_sql = create_user_direct_sql(
                name=org_contact.name,
                email=org_contact.email,
                phone=org_contact.phone,
                temp_password='test123',
            ...
            result['test_creation']['sql'] = bool(test_user_sql)
            if test_user_sql:
                db.session.rollback()  # Don't actually save
...
@main_bp.route("/debug_check_user/<email>")
    ...
    return jsonify(
        {
            ...
            "user_details": (
                {
                    ...
                    "name": user.name if user else None,
                    "role": user.role if user else None,
                    "auditor_profile_id": user.auditor_profile_id if user else None,
                }
                if user
                else None
            ),
            ...
            "org_contact_details": (
                {
                    "contact_id": org_contact.contact_id if org_contact else None,
                    "name": org_contact.name if org_contact else None,
                    "organization_id": (
                        org_contact.organization_id if org_contact else None
                    ),
                }
                if org_contact
                else None
...
@main_bp.route("/debug_contact_columns")
def debug_contact_columns():
    """Check what columns actually exist in OrganizationContacts table"""
    try:
        # Get table info
        result = db.session.execute(
            ...
            WHERE table_name = 'OrganizationContacts' 
        ...
        return jsonify({"table": "OrganizationContacts", "columns": columns})

    except Exception as e:
        return jsonify({"error": str(e)})


@main_bp.route("/debug_login_attempt", methods=["POST"])
def debug_login_attempt():
    """Debug endpoint to test login without redirects"""
    email = request.form.get("email")
    password = request.form.get("password")

    result = {"input_email": email, "input_password": password, "steps": []}

    # Normalize email
    normalized_email = email.strip().lower() if email else ""
    result["normalized_email"] = normalized_email

    # Step 1: Check OrganizationContacts
    org_contact = OrganizationContacts.query.filter(
        db.func.lower(OrganizationContacts.email) == normalized_email,
        OrganizationContacts.is_active == True,
    ).first()
    ...
    if org_contact:
        contact_info = {
            ...
            "name": org_contact.name,
            "email_in_db": org_contact.email,
            "is_active": org_contact.is_active,
            "has_password_hash": bool(org_contact.password_hash),
            "password_match": org_contact.check_password(password),
        }
        result["organization_contact"] = contact_info
    else:
        result["organization_contact"] = {"found": False}

    # Step 2: Check regular Users
    user = Users.query.filter(db.func.lower(Users.email) == normalized_email).first()
    ...
    if user:
        ...
        result["regular_user"] = user_info
    else:
        result["regular_user"] = {"found": False}

    return jsonify(result)


@main_bp.route("/test_contact_login/<email>/<password>")
def test_contact_login(email, password):
    """Test contact login directly"""
    ...
    return jsonify(
        {
            ...
            "get_id": org_contact.get_id(),
            "flask_login_works": hasattr(org_contact, "is_authenticated"),
        }
    )


@main_bp.route("/debug_contact/<email>")
def debug_contact(email):
    """Debug a specific contact"""
    # Try different query methods
    contact_exact = OrganizationContacts.query.filter_by(email=email).first()
    contact_ilike = OrganizationContacts.query.filter(
        OrganizationContacts.email.ilike(email)
    ).first()
    contact_all = OrganizationContacts.query.filter(
        OrganizationContacts.email.ilike("%" + email + "%")
    ).all()
    ...
    result = {
        "input_email": email,
        "exact_match": {
            "found": bool(contact_exact),
            "contact_id": contact_exact.contact_id if contact_exact else None,
            "email_in_db": contact_exact.email if contact_exact else None,
            "name": contact_exact.name if contact_exact else None,
        },
        "ilike_match": {
            "found": bool(contact_ilike),
            "contact_id": contact_ilike.contact_id if contact_ilike else None,
            "email_in_db": contact_ilike.email if contact_ilike else None,
            "name": contact_ilike.name if contact_ilike else None,
        },
        "all_matches": [
            {"contact_id": c.contact_id, "email": c.email, "name": c.name}
...
@main_bp.route("/test_password/<email>/<password>")
def test_password(email, password):
    """Test password for a specific contact"""
    contact = OrganizationContacts.query.filter(
        OrganizationContacts.email.ilike(email)
    ).first()
    ...
    password_match = contact.check_password(password)
    ...
    return jsonify(
        {
            ...
            "password_match": password_match,
            "has_password_hash": bool(contact.password_hash),
        }
    )


@main_bp.route("/debug_contacts")
def debug_contacts():
    """Debug route to check OrganizationContacts data"""
    contacts = OrganizationContacts.query.all()
    result = []
        ...
        result.append(
            {
                ...
                "has_password": bool(contact.password_hash),
                "password_hash_length": (
                    len(contact.password_hash) if contact.password_hash else 0
                ),
...
@main_bp.route("/debug_user")
@login_required
def debug_user():
    """Debug route to check current user information"""
    user_info = {
        "type": type(current_user).__name__,
        "id": getattr(current_user, "id", None),
        "contact_id": getattr(current_user, "contact_id", None),
...
def verify_tfa_login():
    ...
    if request.method == "POST":
        ...
        if str(token) == user.tfa_secret:
            ...
            login_user(user)
            session["session_token"] = new_token
            # Defensive guard added 2026-07-31 (staging), ported 2026-08-09:
...
def extract_all_route(guideline_id):
    ...
    try:
        ...
        task = extract_all_activities_and_tests.delay(guideline_id)
