...
def orgnization():
    ...
    try:
        organizations = Organizations.query.all()  # Fetch all organization data
        if current_user:
        ...
        print(organizations)
        return render_template("org.html", organizations=organizations)
    except Exception as err:
        current_app.logger.error(f"Unexpected error: {str(err)}")
        return jsonify({"error": "Internal server error"}), 500
    ...
    if request.method == "POST":
        try:
            ...
            if current_step == "0":
                ...
                return redirect(url_for("re.edit_re_profile", org_id=org_id))
...
def get_some_activity():
    ...
    try:
        ...
        for i, activity in enumerate(compliance_activities):
            current_app.logger.info(
...
@re_bp.route("/activity/<string:project_name>", methods=["GET"])
@role_required("COMPLIFYRE", "AUDITOR", "RE")
def activity(project_name):
    ...
    # Properly decode URL-encoded project name
    ...
    try:
        # Debug logging
        current_app.logger.info(f"Searching for project: '{project_name}'")

        # Get single project instead of all projects
        project = Projects.query.filter_by(project_name=project_name).first()
        # If not found, try case-insensitive search
        if not project:
            current_app.logger.info(
                f"Exact match not found, trying case-insensitive search"
            )
            project = Projects.query.filter(
                db.func.lower(Projects.project_name) == db.func.lower(project_name)
            ).first()

        if not project:
            # Log all available projects for debugging
            all_projects = Projects.query.with_entities(Projects.project_name).all()
            available_names = [p.project_name for p in all_projects]
            current_app.logger.error(
                f"Project '{project_name}' not found. Available projects: {available_names}"
            )
        ...
        for clause_id, clause_data in unique_clauses.items():
            ...
            clause_status = clause_status_info.get(
                clause_id, get_compliance_status_display_info("no-procedures")
            )
            assessment_status = assessment_status_info.get(
                clause_id, get_assessment_status("no-procedures")
            )

            clause_obj = {
                "id": clause.id,
                "clause": clause,
                "clause_status_info": clause_status,
        ...
        def natural_sort_key(clause_obj):
            clause_no = clause_obj["clause"].clause_no

            # Handle None or empty clause numbers
            if clause_no is None or clause_no == "":
                return [float("inf")]  # Put empty/missing numbers at the end

            # Convert to string and split into parts (numbers and text)
            ...
            for part in parts:
                if part.isdigit():
                    ...
                    # Handle decimal points and other separators
                    if part.strip() in [".", "-", "_"]:
        ...
        for i, clause in enumerate(enriched_clauses):
            current_app.logger.info(f"  {i+1}. {clause['clause'].clause_no}")
...
def evidences():
    """
    Handles the submission of evidence for a specific project control activity.
    It updates the project-specific evidence record with either user-provided
    text or AI-generated content from an uploaded file.
    """
    try:
        # The ID from the form now refers to the ProjectEvidenceArtifact instance
        ...
        if (
            ...
            # --- CORE CHANGE: Use the correct column name ---
        ...
        if file and file.filename:
            if allowed_file(file.filename):
                ...
                full_physical_file_path = os.path.join(UPLOAD_FOLDER_1, filename)
                file_path_for_db = os.path.join(UPLOAD_FOLDER, filename)
                try:
                    file.save(full_physical_file_path)
            ...
            file_path_for_db = uploaded_path
            # Correctly construct the full path for extraction
            full_physical_file_path = os.path.join(
                UPLOAD_FOLDER_1, os.path.basename(uploaded_path)
            )
        ...
        if file_path_for_db:
            try:
                ...
                if not full_physical_file_path.endswith(file_extensions_to_exclude):
                    ...
                    if (
                        ...
                        logger.error(
                            "Vector store upload failed: %s", file_upload_openai
                        )
                        return {
                            "status": "error",
                ...
                # if not content:
                # Still save the file path even if content extraction fails
                if file_path_for_db:
                    artifact.evidence_file_path = file_path_for_db
                    db.session.commit()
                    # flash("Could not extract content, but the file was saved.", "warning")
                    # return redirect(request.referrer)
            except Exception as e:
            ...
            try:
                # prompt = prompt_get_evidence_answer(project_evidence_id, evidence_item, clause_description, content)
                ...
                try:
                    ...
                    print(ai_answer)
                    if ai_answer:
                        # --- CORE CHANGE: Use the correct column names ---
                        artifact.evidence_text = ai_answer
                        artifact.evidence_file_path = file_path_for_db
                    else:
                        flash("AI response was empty. No autofill.", "warning")
                        if file_path_for_db:
                            artifact.evidence_file_path = file_path_for_db
                except (json.JSONDecodeError, AttributeError):
                    flash(
                        f"Failed to parse AI response. Raw response: {res}...",
                        "warning",
                    )
                    if file_path_for_db:
                        artifact.evidence_file_path = file_path_for_db
                except Exception as ai_e:
                    flash(f"Error processing AI answer: {ai_e}", "warning")
                    if file_path_for_db:
                        artifact.evidence_file_path = file_path_for_db

            except Exception as e:
                flash(f"Error generating answer: {e}", "danger")
                if file_path_for_db:
                    artifact.evidence_file_path = file_path_for_db
        else:
    ...
    try:
        ...
        for item in data:
            ...
            try:
                ...
                d1["Guideline Extract"] = guideline_data.get("DocumentDetails", {}).get(
                    "DocumentName", "N/A"
                )
            except Exception:
                d1["Guideline Extract"] = "N/A"
...
@re_bp.route("/report_options/<string:project_name>", methods=["GET"])
def report_options(project_name):
    """
    ...
    try:
        ...
        return render_template(
            "dashboards/auditor/report_options.html", project=project
        )
...
@re_bp.route("/generate_report/<string:project_name>", methods=["POST"])
def generate_audit_report(project_name):
    """
    ...
    try:
        ...
        if project.free_report_used:
            flash(
                "You have already used your free report for this project. Please contact CompliFyre@crackerjacktech.com for additional reports.",
                "warning",
            )
            return redirect(url_for("re.report_options", project_name=project_name))
        # Get form data
        ...
        for clause in clauses:
            ...
            observation_data["Clause Name"] = clause.clause_text

            # Guideline Extract - Get actual guideline name from ProjectGuideline
            guideline_extract = get_guideline_extract(clause)
            observation_data["Guideline Extract"] = guideline_extract
        ...
        if report_format == "sectional":
            ...
            template_docx = "app/routes/re/Fictional audit Template.docx"
            output_doc_path = f"uploads/audit_report/{project_name}_Consolidated_Audit_Report_{timestamp}.docx"
            full_path = os.path.join(BASE_DIR, output_doc_path)
            os.makedirs(os.path.dirname(full_path), exist_ok=True)

            process_audit_observations(
                template_docx, full_path, consolidated_observations, documentation_data
            )
            ...
            # Generate Excel file
            output_excel_path = f"uploads/audit_report/{project_name}_Consolidated_Audit_Report_{timestamp}.xlsx"
...
def clause_applicability_status():
    ...
    Returns ONLY applicability status - no compliance data
    """
    try:
        # Accept JSON or form
        payload = {}
        ...
        # If client supplied explicit applicability, use it; otherwise toggle
        explicit = payload.get("applicability")
        if explicit is not None:
            # handle JSON boolean or string "true"/"false"
            if isinstance(explicit, str):
                explicit_lower = explicit.lower()
                if explicit_lower in ("true", "1", "yes"):
                    new_val = True
                elif explicit_lower in ("false", "0", "no"):
                    new_val = False
                else:
                    return (
                        jsonify(status="error", message="Invalid applicability value"),
                        400,
                    )
            else:
                new_val = bool(explicit)
        else:
            new_val = not bool(clause.applicability)

        clause.applicability = new_val

        db.session.add(clause)
        db.session.commit()

        # CRITICAL: Return ONLY applicability data, no compliance status
        current_app.logger.info(
            f"User {getattr(current_user, 'id', None)} set clause {clause_id} applicability to {new_val}"
        )

        return (
            jsonify(
                status="success",
                applicability=new_val,
                clause_id=clause.id,
                message=f"Applicability set to {'Applicable' if new_val else 'Not Applicable'}",
            ),
            200,
        )

    except Exception as e:
        current_app.logger.exception("Failed to update clause applicability")
        db.session.rollback()
