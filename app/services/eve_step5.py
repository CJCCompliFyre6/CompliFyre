...
def _build_step5_prompt(
    ...
    """Build EVE Step 5 prompt — focused, traceable, checklist-driven."""

    org_industry = (org_context or {}).get("industry_type", "Not specified")
    org_type = (org_context or {}).get("organization_type", "Not specified")
    return f"""You are an Audit Evidence Evaluation Engine.

TASK:
Evaluate the provided evidence against the checklist items below.
Be factual, traceable, and concise. Do NOT hallucinate. Do NOT invent findings.
Return ONLY valid JSON. No explanation. No markdown.
...
STEP 1 — EXTRACT EVIDENCE METADATA:
Extract from the evidence document:
- entity_name: name of the organization that issued/owns this document
- document_title: title of the document
- approval_authority: who approved it (if mentioned)
- approval_date: date of approval (if mentioned)
- effective_date: date from which it is effective (if mentioned)
- document_version: version number (if mentioned)
If any field is not found in the document, return empty string for that field.

---

STEP 2 — RUN ADMISSIBILITY TESTS:
Run each test independently. For each test return PASS or FAIL with a specific plain-English reason.

TEST 1 — ORGANIZATION_MATCH:
Does the entity_name in the document match the auditee "{auditee_name}"?
- PASS: entity name matches or is clearly the same organization
- FAIL: entity name is different, missing, or belongs to another organization
- reason: state exactly what entity name was found and why it passed or failed

TEST 2 — PERIOD_ALIGNMENT:
Does the evidence cover the audit period {audit_period_start} to {audit_period_end}?
Apply dimension-aware rules:
- DESIGN items: document effective during audit period OR approved/created before audit period start = PASS
- IMPLEMENTATION items: rollout completed before or during audit period = PASS
- OPERATING items: execution must fall WITHIN the audit period
- FAIL only if evidence clearly falls outside the applicable period for its dimension
- If audit period is "Unknown" — always PASS this test
- reason: state the document date/period found and explain specifically why it passed or failed

TEST 3 — DOCUMENT_AUTHENTICITY:
Is the document genuine and complete?
- PASS: document has clear title, content is coherent and complete
- FAIL: document is corrupt, empty, truncated, or clearly fabricated
- reason: brief explanation

TEST 4 — RELEVANCE_TO_ACTIVITY:
Is this document relevant to the checklist items being tested?
- PASS: document content is clearly relevant to at least one checklist item
...
For EACH checklist item in the checklist provided, search the evidence document carefully and report:

- checklist_id: the item ID from the checklist
- found: FOUND / PARTIAL / NOT_FOUND
  * FOUND: clear explicit evidence in the document satisfying the requirement
  * PARTIAL: some relevant content exists but does not fully satisfy the requirement
  * NOT_FOUND: no relevant content found for this item in this document
- location: exact section, heading, paragraph where found (if FOUND or PARTIAL). Empty string if NOT_FOUND.
- extract: verbatim relevant text from the document (max 200 words). Empty string if NOT_FOUND.
- gap: specific description of what is missing or insufficient. Empty string if FOUND.
- signal: SUPPORTS (if FOUND) / INSUFFICIENT (if PARTIAL or NOT_FOUND) / CONTRADICTS (only if evidence directly contradicts the requirement)
- basis: one sentence explaining the signal decision

RULES:
1. Evaluate ONLY what is explicitly present in the document. Do NOT assume or infer.
2. Illustrative language in checklist ("such as", "including", "for example") — do NOT fail for absence of examples.
3. Do NOT mark NOT_FOUND simply because a date precedes the audit period for DESIGN/IMPLEMENTATION items.
4. If document is INADMISSIBLE — mark all items as NOT_FOUND with gap = "Evidence inadmissible: [specific reason]"
5. Every checklist item must have an entry — do not skip any.
...
{{{{
  ...
  "admissibility_tests": [
    {{{{
      "test": "ORGANIZATION_MATCH",
      "result": "PASS | FAIL",
      "reason": ""
    }}}},
    {{{{
      "test": "PERIOD_ALIGNMENT",
      "result": "PASS | FAIL",
      "reason": ""
    }}}},
    {{{{
      "test": "DOCUMENT_AUTHENTICITY",
      "result": "PASS | FAIL",
      "reason": ""
    }}}},
    {{{{
      "test": "RELEVANCE_TO_ACTIVITY",
      "result": "PASS | FAIL",
      "reason": ""
    }}}}
  ],
  "checklist_evaluation": [
    {{{{
      ...
      "extract": "",
      "gap": "",
      "signal": "SUPPORTS | INSUFFICIENT | CONTRADICTS",
      "basis": ""
    }}}}
  ],
  "item_signals": [
  ...
  "results": [
    {{{{
      "checklist_id": "",
      "status": "YES | PARTIAL | NO | NEEDS_REVIEW",
...
* Do NOT invent content not present in the evidence
* Every checklist item must appear in checklist_evaluation, item_signals, and results
* item_signals and results must mirror checklist_evaluation entries
"""
