        ...
        for clause in clauses:
            ...
            with session_scope() as session:
                ...
                for index, act in enumerate(activities, start=1):
                    ...
                    # Generate EVE checklist -- synchronous, must complete before clause is
                    # marked done. Restored here from the now-deleted, orphaned
                    # extract_all_activities_and_tests, which had this correct but was never
                    # actually called by any live route -- this function (used by both real
                    # buttons) was missing it entirely. Build Sequence #382.
                    try:
                        from app.services.eve_tasks import generate_control_checklist
                        from app.models.ai import ControlActivity as _CtrlAct
