...
def extract_clauses(self, guideline_id: int):
    ...
    try:
        ...
        for start_page in range(1, total_pages + 1, page_chunk_size):
            ...
            context_text = extract_context_text(file_path, start_page, end_page)
...
def extract_all_activities_and_tests(self, guideline_id: int):
    ...
    try:
        ...
        for clause in clauses:
            ...
            # Check if clause already has compliance activities
            ...
            if existing_activities:
                logger.info(
                    "Skipping clause %s (id=%s) - already has compliance activities",
                    clause_number,
                    clause_id_val,
                )
                ...
                # Update progress for skipped clauses
                progress = (
                ...
                update_compliance_progress(
                    ...
                    "PROCESSING",
                    int(progress),
                    f"Processing clauses... ({processed_clauses}/{total_clauses})",
                    processed_clauses,
                    total_clauses,
                )
                continue
            ...
            logger.info(
                "Processing Clause(id=%s, number=%s)", clause_id_val, clause_number
            )
...
def extract_selected_activities_and_tests(self, guideline_id: int, clause_ids: list):
    ...
    try:
        ...
        for clause in clauses:
            ...
            logger.info(
                "Processing selected Clause(id=%s, number=%s)",
                clause_id_val,
                clause_number,
            )
