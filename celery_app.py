...
def celery_init_app(app: Flask) -> Celery:
    ...
    celery_app.config_from_object({
        "broker_url": f'redis://:{redis_password}@{redis_host}:6379/0' if redis_password else f'redis://{redis_host}:6379/0',
        "result_backend": f'redis://:{redis_password}@{redis_host}:6379/0' if redis_password else f'redis://{redis_host}:6379/0',
        "task_routes": {
        ...
        "broker_transport_options": {"visibility_timeout": 21600},  # fix 2026-08-01: ported from
        # staging, which already had this -- 6 hours, matching staging exactly. task_acks_late +
        # long-running bulk tasks (e.g. activity-generation) need this longer than Redis's 1hr
        # default, or the broker redelivers an in-progress task to another worker prematurely.
